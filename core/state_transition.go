...
func (st *StateTransition) buyGas() error {
	...
	// TODO: opera won't support Cancun because it's not compability
	// So for the time being, it's not neccessary to check this
		...
		if !st.evm.Config.NoBaseFee || st.gasFeeCap.BitLen() > 0 || st.gasTipCap.BitLen() > 0 {
			...
			if st.gasFeeCap.Cmp(st.gasTipCap) < 0 {
				return fmt.Errorf("%w: address %v, maxPriorityFeePerGas: %s, maxFeePerGas: %s", ErrTipAboveFeeCap,
					st.msg.From().Hex(), st.msg.GasTipCap(), st.msg.GasFeeCap())
			}
			// This will panic if baseFee is nil, but basefee presence is verified
			// as part of header validation.
			if st.gasFeeCap.Cmp(st.evm.Context.BaseFee) < 0 {
				return fmt.Errorf("%w: address %v, maxFeePerGas: %s baseFee: %s", ErrFeeCapTooLow,
					st.msg.From().Hex(), st.msg.GasFeeCap(), st.evm.Context.BaseFee)
			}
		}
	}
...
func (st *StateTransition) TransitionDb() (*ExecutionResult, error) {
	...
	if contractCreation {
		ret, _, st.gas, vmerr = st.evm.Create(sender, msg.Data(), st.gas, msg.Value())
	} else {
	...
	log.Info("Transaction execution ends", "gas", st.gasUsed())
...
// TODO: opera don't need to support BlobHashes
